...
%   harmonics for a particular value of N. Allows the use of four symmetry
...
%   num2str(N).mat
%   
% N - principal order of the harmonics to be symmetrized
...
transform = clebschgordan(N)*complexreal(N);
...
if any(col)
    disp(['Rounding error: ',num2str(max([max(max(abs(Ra*S-S))),max(max(abs(Rb*S-S))),max(max(abs(Rc*S-S))),max(max(abs(Rd*S-S)))]))]);
